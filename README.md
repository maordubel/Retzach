...
📐 **[מדריך המותג → BRAND.md](BRAND.md)** · 🖤 **[שפת העיצוב → DESIGN-PHILOSOPHY.md](DESIGN-PHILOSOPHY.md)** · ✍️ **[קופי → COPY.md](COPY.md)**

---

## מה יש בפנים

- **ארכיון** — 36 פרקים **מקובצים לפי עונות עם כותרות דביקות**, חיפוש חופשי,
  גיליון עונות לניווט מהיר, סימון "תיק סגור" מול "בקרוב"
- **תפריט תחתון קבוע** בנייד (תיקים · עונות · המשחק · חיפוש · אודות) — עם `env(safe-area-inset)`
...
- **"תיק פתוח" · חדר המשחקים** — שלושה משחקים, מנוע אחד:
  **החקירה** — תיק חי: מקבלים אותו ביום שהוא נפתח, שבע החלטות חקירה בזמן אמת
  שנמדדות מול מה שהמשטרה **באמת** עשתה, לידים שפותחים ראיות, חיפוש חופשי בכל
  הארכיון (אין רשימת חשודים), וסיום ב**חדר ריאיון** בסגנון דוקו עם
  **חתימת תיק גנרטיבית** ייחודית לכל משחק — שניתן לשתף כתמונה 1080×1920 לסטורי/טיקטוק.
  — עם שכבת רקע חיה, חוט אדום בין הראיות וחותמת שנה. לצידו
  *"איזה רוצח סדרתי אתה?"* ו*"על ידי מי היית נרצח?"*.
  המנוע **גוזר פרופיל מכל תיק לבד** מהטקסט ומהנתונים, ולכן
  **תיק חדש מצטרף לשלושת המשחקים ברגע שהוא נכנס למאגר** — בלי לגעת בקוד המשחק
...
│   └── app.js                 ראוטינג, רינדור, אינטראקציות, dataLayer
...
├── assets/games.js            "תיק פתוח" — שלושת המשחקים (נטען רק בלחיצה)
├── DESIGN-PHILOSOPHY.md       Evidentiary Silence — שפת העיצוב
├── COPY.md                    מסמך הקופי · המדף, הקרדיט ו-TakeMeOut!
├── plate.png                  לוח הארכיון · Plate I
...
### סנכרון מספרים — אוטומטי

המספר בכרטיס הנתונים ("5 קורבנות") **נגזר מרשימת הקורבנות עצמה**, לא נכתב ידנית.
כל רשומה מקבלת תפקיד — `v` קורבן · `m` נעדר · `s` ניצול/ה · `o` דמות נוספת בתיק —
והלשונית מתפצלת לסקשנים לפי התפקיד, כל אחד עם המונה שלו.
רשומה שמייצגת קבוצה (למשל "עשרים האישומים שהוקפאו") מקבלת `group: N`.
כשהגיליון מציין מספר גדול יותר ממה שמתועד בשם — מוצגת אוטומטית הערה שמסבירה את הפער.
**לכן אי אפשר יותר שהגרפיקה והמידע יסתרו זה את זה.**
